...
def is_playlist_url(url):
    # BUG FIX: URLs with list= param are also playlists
...
def _blocking_playlist_info(url):
    # BUG FIX: remove playlist_items="0" — this was only for info fetch,
    # setting "0" in playlist means entries don't come at all!
    opts = _info_opts()
    opts.pop("playlist_items", None)   # ← CRITICAL FIX
    opts.pop("format", None)           # format not needed for playlist info
    opts.update({
        "extract_flat":  True,
        "playlistend":   MAX_PLAYLIST,
        "ignoreerrors":  True,         # so one failed video doesn't stop the whole playlist
    })
    try:
...
async def upload_file(client, chat_id, result, fmt, smsg):
    ...
    # 🎯 WZML-X Style Caption: Only filename shows now
...
async def ping_cmd(client, message):
    ...
    await m.edit_text(
        ...
        f"🍃 MongoDB: `{'✅' if db.enabled else '❌'}`\n"
        f"🔌 Proxy: `{'✅' if PROXY_URL else '❌'}`\n"
        f"📂 Sessions: `{len(URL_SESSIONS)+len(PL_SESSIONS)}`"
    )

@app.on_message(filters.document & filters.private)
async def cookies_upload_cmd(client, message):
    global COOKIES_FILE
    if message.from_user.id not in ADMINS:
        return
    dest = os.path.join(BASE_DIR, "cookies.txt")
    m = await message.reply_text("⬇️ **Saving cookies...**")
    try:
        await message.download(file_name=dest)
        with open(dest, "r", encoding="utf-8", errors="ignore") as f:
            data = f.read()
        COOKIES_FILE = dest
        ok = await db.save_cookies(data)
        await _safe_edit(m, "✅ **Cookies updated!**\n" + ("🍪 Synced to MongoDB." if ok else "⚠️ MongoDB not configured, saved locally only."))
    except Exception as e:
        await _safe_edit(m, f"❌ Failed: `{e}`")
...
async def handle_url(client, message):
    ...
    if uid in WAITING_SEL:
        ...
        if e:
            ...
            smsg = await message.reply_text(f"✅ `{len(indices)}` selected. Quality...")
            loop = asyncio.get_event_loop()
...
async def quality_cb(client, query: CallbackQuery):
    ...
    if not e:
        await query.answer("⚠️ Session expired! Send link again.", show_alert=True); return
...
async def _start_dl(uid, qual, e, query, client):
    ...
    smsg = await query.message.reply_text(f"⚙️ **Download starting...**\n🎞️ `{qual[:50]}`")
...
async def _dl_single(url, qual, info, smsg, client, chat_id):
    ...
    await _safe_edit(smsg, "⬇️ **Download starting...**")
...
async def main():
    global COOKIES_FILE
    if db.enabled:
        data = await db.get_cookies()
        cookies_path = os.path.join(BASE_DIR, "cookies.txt")
        if data:
            with open(cookies_path, "w", encoding="utf-8") as f:
                f.write(data)
            COOKIES_FILE = cookies_path
            logger.info("✅ Cookies loaded from MongoDB")
        elif COOKIES_FILE:
            with open(COOKIES_FILE, "r", encoding="utf-8", errors="ignore") as f:
                await db.save_cookies(f.read())
            logger.info("✅ Local cookies.txt synced to MongoDB")
    else:
        logger.info("MongoDB not configured (MONGO_URI unset) — cookies stay local only")
